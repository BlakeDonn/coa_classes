...
window.COA_COPY = (() => {
  ...
  const CLASS_ENGINE = {
    ...
    venomancer: {
      label: "The engine — forms and stacks",
      lede: "Each spec fights in its own form and builds its own stacks; two active Venoms are the only layer they share.",
      text: "Build the spec's layer and spend it on that spec's payoff: ten Exposed Flesh, five Brood Marks, ten Fungal Growth, or Shadra's Vigil. The two Venoms stay active and fire from your casts and strikes.",
    },
    "witch-doctor": {
      label: "The engine — Spirits",
      lede: "Collected Spirits stack to five: hold them and every payoff scales, or unleash the whole stack at once.",
      text: "Collect Spirits from damage and healing; each one regenerates health and mana for 12 seconds. Each spec adds a second value beside the stack — stored Thread damage, brewed Ingredients, or Voodoo Hunger.",
    },
    pyromancer: {
      label: "The engine — Embers and Heat",
      lede: "Every Pyromancer runs two resources: Embers you bank and spend, and Heat that pays out the moment you generate it.",
      text: "Generate Embers with Lava Shard and spend them one at a time. Generating Heat has a 25% chance per cast to grant Flamecasting — 5% spell haste for 15 seconds, five stacks deep.",
    },
    runemaster: {
      label: "The engine — engravings",
      lede: "Weapon Engravings land on a trigger chance you buy up with cooldowns; each spec counts its own sequence on top.",
      text: "Engravings trigger from your weapons on a chance, and Zenith doubles that chance for 6 seconds. Runic Tattoos cut resource costs by 10%, always.",
    },
    necromancer: {
  ...
  const VERBS = {
    ...
    "venomancer/venom": ["Fester", "Grow Fungal Growth to ten; every stack feeds the rot, and Mycosis hits 20% harder per stack."],
    "venomancer/stalking": ["Stalk", "Bank five Brood Marks in Spider Form; Facemelter and Widowmaker scale with every mark."],
    "venomancer/fortitude": ["Shed", "Carry the Exposed Flesh to ten, then shed it — the clear pays damage reduction the longer it held."],
    "venomancer/vizier": ["Prime", "Prime Shadra's Vigil across five heals; each one lands 10% stronger."],
    "witch-doctor/voodoo": ["Snap", "Store ten seconds of damage in the Threads, then Hexfire snaps it all at once."],
    "witch-doctor/shadowhunting": ["Gather", "Hold Spirits while Hunger climbs beside them; Spirit Eclipse unleashes every one you kept."],
    "witch-doctor/brewing": ["Mix", "Brew the Ingredients, then the Cauldron makes the party's healing land 20% stronger."],
    "pyromancer/flameweaving": ["Reserve", "Sunstrider Array banks an Ember every three seconds and doubles Cinderheart's Heat."],
    "pyromancer/incineration": ["Ripen", "Let the burns mature — Fired Up makes spenders instant, refunds 30%, and crits 20% more."],
    "pyromancer/draconic": ["Invoke", "Five Embers call the Invocation, and every Heat you generate pulls its cooldown closer."],
    "runemaster/runic": ["Brand", "Mark the target for eight seconds; the next Runeblade erupts onto everyone nearby."],
    "runemaster/arcane": ["Escalate", "Sequence three glyphs, then Glyphic Ruin releases the active one with full spell power on top."],
    "runemaster/riftblade": ["Beat", "Every third Runeblade lands 30% harder and transforms your next cast — count the beat."],
